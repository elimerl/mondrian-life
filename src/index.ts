...
const game = new Game(
  Array.from({ length: Math.floor((window.innerWidth * 3) / 75) }, () => {
    return Array.from(
      { length: Math.floor((window.innerHeight * 3) / 75) },
      () => {
        return 0;
      },
    );
...
function set(x: number, y: number, v: 1 | 0) {
  game.board[y][x] = v;
}
set(1, 0, 1);
set(2, 1, 1);
...
const colors = Array.from(
  { length: Math.floor((window.innerWidth * 3) / 75) },
  () => {
    return Array.from(
      { length: Math.floor((window.innerHeight * 3) / 75) },
      () => {
